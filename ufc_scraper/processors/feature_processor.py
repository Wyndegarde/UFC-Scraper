...
class FeatureProcessor(DataFrameABC):
    def __init__(self, csv_path, allow_creation) -> None:
        ...
        self.fighters = np.unique(np.concatenate([self.object_df['red_fighter'],self.object_df['blue_fighter'] ], axis = None))
        self.percent_stats = [column for column in self.object_df.columns if '%' in column]
    ...
    def _build_regression_df(self) -> pd.DataFrame:
        ...
        for column in self.percent_stats:
            each_stat_df = pd.DataFrame(columns = [column, "Y_" + column, "X_" + column]) # Builds a dataframe for each stat.
    ...
    def migration_placeholder(self):
        ...
        for fighter in self.fighters:
            fighter_df = self._get_fighter(self.object_df,fighter).sort_values(by = 'date', ascending = True) # Gets each fighter and orders their fights, earliest to most recent.
